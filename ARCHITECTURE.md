...
1. **의존성 설치 단계**: Node.js 환경에서 npm을 통해 pnpm을 전역 설치한 후, pnpm을 사용하여 필요한 패키지를 설치합니다. `--no-verify-store-integrity` 옵션을 적용하여 패키지 서명 검증 관련 문제를 방지합니다.
2. **빌드 단계**: 설치된 의존성을 사용하여 Next.js 애플리케이션을 빌드합니다. 여기서도 npm을 통해 pnpm을 전역 설치하고 `--no-verify-store-integrity` 옵션을 적용합니다.
